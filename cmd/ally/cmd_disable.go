...
func init() {
	var cmd = &cobra.Command{
		Use:   "disable {app}",
		Run:   runDisable,
		Short: "Disable an app that will not be start by wakeup",
		Long:  "Disable an app that will not be start by wakeup",
...
func runDisable(cmd *cobra.Command, args []string) {
	...
	if app.Info.Ephemeral {
		Exit(2, "ERROR[Ally]: Can not disable ephemeral app[%s]\n", app.Info.Name)
	}
